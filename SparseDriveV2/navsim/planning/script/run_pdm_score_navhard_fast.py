...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]], pdm_score_fn) -> List[pd.DataFrame]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_one):
        ...
        try:
            ...
            score_row_stage_one, ego_simulated_states = pdm_score_fn(
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_two):
        ...
        try:
            ...
            score_row_stage_two, ego_simulated_states = pdm_score_fn(
                metric_cache=metric_cache,
                model_trajectory=trajectory,
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
...
def main(cfg: DictConfig) -> None:
    ...
    # import pickle
    # with open("/job_data/predictions", "wb") as f:
    #     pickle.dump(merged_predictions, f)
    # exit()
    ...
    pdm_score_df.to_csv(save_path / "navhard.csv")
    ...
    logger.info(
        ...
        Finished running evaluation.
            ...
            Results are stored in: {save_path / "navhard.csv"}.
        """
    )
    ...
    save_path = Path(cfg.output_dir)
    timestamp = datetime.now().strftime("%Y.%m.%d.%H.%M.%S")
    pdm_score_df.to_csv(save_path / "navhard_bug_fix.csv")
    ...
    logger.info(
        ...
        Finished running evaluation.
            ...
            Number of failed scenarios: {num_failed_scenarios}.
            Final extended pdm score of valid results: {pdm_score_df[pdm_score_df["token"] == "extended_pdm_score_combined"]["score"].iloc[0]}.
            Results are stored in: {save_path / "navhard_bug_fix.csv"}.
        """
    )
